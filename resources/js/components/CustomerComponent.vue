            ...
            <div class="col-md-12">
                <div class="card shadow-sm rounded-lg border-0">
                    <div class="card-header bg-white border-0 d-flex align-items-center justify-content-between" style="min-height: 70px;">
                        <h4 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.7rem; letter-spacing: 0.5px;">
                            Customers <span class="mx-2">|</span> <span class="badge bg-primary text-white ms-2" style="font-size: 1rem;">{{total}}</span>
                        </h4>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            <div class="row align-items-center g-2">
                                ...
                                <div class="col-md-3">
                                    <select v-model="queryFiled" class="form-control rounded-pill" id="fileds">
                                ...
                                <div class="col-md-5">
                                    <input v-model="query" type="text" class="form-control rounded-pill" placeholder="Search">
                                </div>
                                <div class="col-md-2">
                            ...
                            <table class="table table-hover table-borderless align-middle mb-0 overflow-hidden" style="border-radius: 1.2rem; background: #fafbfc;">
                                <thead class="table-light" style="border-radius: 1.2rem 1.2rem 0 0;">
                                    <tr style="font-size: 1rem; border-radius: 1.2rem 1.2rem 0 0; overflow: hidden;">
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; border-top-left-radius: 1.2rem; border-bottom: 1.5px solid #e5e7eb; background: #f8f9fa;">Sl</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Name</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Company</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Email</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Phone</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Address</th>
                                ...
                                <tbody>
                                    <tr v-show="customers.length" v-for="(customer) in customers" :key="customer.id" class="bg-white border-bottom border-light" style="vertical-align: middle; transition: background 0.2s;">
                                        <th scope="row" class="text-muted text-start">{{ customer.id}}</th>
                                        <td class="text-start">{{ customer.c_name }}</td>
                                        <td class="text-start">{{ customer.company_name.length>16 ? customer.company_name.substring(0,16)+" ..." : customer.company_name }}</td>
                                        <td class="text-start">{{ customer.email.length >16 ? customer.email.substring(0,16)+" ..." : customer.email }}</td>
                                        <td class="text-start">{{ customer.phone }}</td>
                                        <td class="text-start">{{ customer.address.length > 16 ? customer.address.substring(0,16)+" ..." : customer.address }}</td>
                                        <td class="text-start">
                                            <button type="button" @click.prevent="show(customer)" class="btn btn-light border-0 btn-sm rounded-circle me-1 shadow-none" title="View">
                                                <i class="fas fa-eye text-primary"></i>
                                            </button>
                                            <button type="button" @click.prevent="edit(customer)" class="btn btn-light border-0 btn-sm rounded-circle me-1 shadow-none" title="Edit">
                                                <i class="fas fa-edit text-success"></i>
                                            </button>
                                            <button type="button" @click.prevent="destroy(customer)" class="btn btn-light border-0 btn-sm rounded-circle shadow-none" title="Delete">
                ...
                <div class="modal fade" id="customerModalLong" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        ...
                        <div class="modal-header">
                            <h5 v-if="showMode == true" class="modal-title text-info" id="exampleModalLongTitle">Details for <strong>{{ showDetails.c_name }}</strong> </h5>
                            <h5 v-else class="modal-title text-primary" id="exampleModalLongTitle">{{ editMode ? "Edit" : "Add New"}}  Customer</h5>
                        ...
                        <div class="modal-body">
                          <alert-error :form="form" message="There were some problems with your input."></alert-error>
                                ...
                                <input v-model="form.c_name" type="text" name="c_name"
                                    class="form-control rounded-pill" :class="{ 'is-invalid': form.errors.has('c_name') }">
                                ...
                                <input v-model="form.company_name" type="text" name="company_name"
                                    class="form-control rounded-pill" :class="{ 'is-invalid': form.errors.has('company_name') }">
                                ...
                                <input v-model="form.email" type="email" name="email"
                                    class="form-control rounded-pill" :class="{ 'is-invalid': form.errors.has('email') }">
                                <has-error :form="form" field="email"></has-error>
                                </div>
                                <div class="form-group">
                                <label>Customer Phone</label>
                                <input v-model="form.phone" type="tel" name="phone"
                                    class="form-control rounded-pill" :class="{ 'is-invalid': form.errors.has('phone') }">
                                <has-error :form="form" field="phone"></has-error>
                                </div>
                                <div class="form-group">
                                <label>Customer Address</label>
                                    <textarea v-model="form.address" name="address" rows="3" class="form-control rounded-4"
                        ...
                        <div class="modal-footer">
                            ...
                            <button :disabled="form.busy" type="submit" class="btn btn-primary rounded-pill">Save</button>
                        </div>
                         </form>
