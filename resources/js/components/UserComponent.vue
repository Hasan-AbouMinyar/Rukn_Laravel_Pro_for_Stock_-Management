                ...
                <div class="card shadow-sm rounded-lg border-0">
                    <div class="card-header bg-white border-0 d-flex align-items-center justify-content-between" style="min-height: 70px;">
                        <h4 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.7rem; letter-spacing: 0.5px;">
                            Employee <span class="mx-2">|</span> <span class="badge bg-primary text-white ms-2" style="font-size: 1rem;">{{total}}</span>
                        </h4>
                            ...
                            <table class="table table-hover table-borderless align-middle mb-0 overflow-hidden" style="border-radius: 1.2rem; background: #fafbfc;">
                                <thead class="table-light" style="border-radius: 1.2rem 1.2rem 0 0;">
                                    <tr style="font-size: 1rem; border-radius: 1.2rem 1.2rem 0 0; overflow: hidden;">
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; border-top-left-radius: 1.2rem; border-bottom: 1.5px solid #e5e7eb; background: #f8f9fa;">Employee Id</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Employee Name</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Employee E-mail</th>
                                        <th scope="col" class="fw-medium text-dark py-3 border-0 border-bottom text-start" style="letter-spacing:0.02em; background: #f8f9fa;">Employee Type</th>
                                ...
                                <tbody>
                                    <tr v-for="user in users" :key="user.id" class="bg-white border-bottom border-light" style="vertical-align: middle; transition: background 0.2s;">
                                        <th scope="row" class="text-muted text-start">{{ user.id }}</th>
                                        <td class="text-start">{{ user.name }}</td>
                                        <td class="text-start">{{ user.email }}</td>
                                        <td class="text-start">{{ user.is_admin ? 'Admin' : 'Employee' }}</td>
                                        <td class="text-start">
                                            <button type="button" @click.prevent="show(user)" class="btn btn-light border-0 btn-sm rounded-circle me-1 shadow-none" title="View">
                                                <i class="fas fa-eye text-primary"></i>
                                            </button>
                                            <button type="button" @click.prevent="edit(user)" class="btn btn-light border-0 btn-sm rounded-circle me-1 shadow-none" title="Edit">
                                                <i class="fas fa-edit text-success"></i> 
                                            </button>
                                            <button type="button" @click.prevent="destroy(user)" class="btn btn-light border-0 btn-sm rounded-circle shadow-none" title="Delete">
                    ...
                    <div class="modal-dialog" role="document">
                        ...
                        <div class="modal-body">
                          <alert-error :form="form" message="There were some problems with your input."></alert-error>
                                ...
                                <div class="form-group">
                                <label>Role</label>
                                <select v-model="form.is_admin" name="is_admin" class="form-control">
                              ...
                              <div class="card border-info mb-3">
                                <div class="card-body">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        ...
                                        <li class="list-group-item"><strong>Role : </strong> {{ showDetails.is_admin ? 'Admin' : 'Employee' }}</li>
                        ...
                        buttons:[
                            {
                                ...
                                action : toast => {
                                    ...
                                    .then(res=>{
                                        this.getData()
                                            this.$Progress.finish();
                                            this.$snotify.success('Employee Delete Successfully','Success')
                    ...
                    update() {
                        ...
                        this.form.put('/api/user/' + this.form.id)
                            .then(res => {
                                ...
                                if (this.form.successful) {
                                    this.$Progress.finish();
                                    this.$snotify.success('Employee updated successfully', 'Success');
                                } else {
                                    this.$Progress.fail();
